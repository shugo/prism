...
if enable_config("parsey", parsey_default)
  unless File.exist?(File.expand_path("../../src/parsey/parse.c", __dir__))
    abort "src/parsey/parse.c has not been generated; run `bundle exec rake parsey:generate` (it needs the lrama gem)"
  end
...
# src/parsey/parse.c is generated by lrama (`rake parsey:generate`); VPATH
# exposes the grammar next to it, so cancel make's builtin yacc rules or make
# may try to send the grammar to bison.
